...
def analyze_experiments():
    ...
    for cell_name, cell_data in cells.items():
        ...
        stats["avg_wait_and_observe"] = (
            sum(wait_counts) / len(wait_counts) if wait_counts else 0
        )
        stats["sd_wait_and_observe"] = (
            statistics.stdev(wait_counts) if len(wait_counts) > 1 else 0
        )
            ...
            output[cell_name] = {
                ...
                "tool_usage": {
                    ...
                    "percentages": {
                        ...
                        if sum(stats["tool_usage"].values()) > 0
                        else 0
    ...
    for cell_name, stats in sorted(output.items()):
        ...
        print(
            f"    - Range: {stats['budget_stats']['min']} - {stats['budget_stats']['max']}"
        )
        print(f"  Tool Usage (per-run mean ± SD):")
        for tool, tool_stat in sorted(
            stats["tool_usage"]["per_tool_stats"].items(), key=lambda x: -x[1]["total"]
        ):
        ...
        print(
            f"    - Mean ± SD: {stats['wait_and_observe']['mean']} ± {stats['wait_and_observe']['sd']}"
        )
        print(
            f"    - Range: {stats['wait_and_observe']['min']} - {stats['wait_and_observe']['max']}"
        )
